...
from src.c_generate_soft.utils import setup_logger, MetricLogger, strip_prefix_if_present
...
from src.c_generate_soft.validate import validate
...
def train(args, logger):
    ...
    args.num_vocab = len(train_loader.weight)  # number of words
...
def parse_args():
    ...
    parser.add_argument('--w_stop', type=float, default=1, help='weight of stop loss')
    ...
    args.save_dir = os.path.join(args.generator_dir, args.category, 'trained_generate_models', topic_num, title_num)

    """
     args.data_path = "/data/text/{}/{}/{}/id.pkl".format(title_num,topic_num,args.category)
     ...
     if (args.reload_ckpt) and (args.is_coverage):
        ...
        args.save_dir = "/data/generate_models/{}/{}_lr1e-7/{}".format(title_num, topic_num, args.category)
    """
